...
XPLMDataRef gIgnitersDataRef = NULL;
XPLMDataRef gStarterDurationDataRef = NULL;
XPLMDataRef gNumberOfEnginesDataRef = NULL;

// Descriptions for the Input Text widget
char DataRefDesc[MAX_NUMBER_ENGINES][20] = {"1", "2", "3", "4", "5", "6", "7", "8"};

float EngineStarterLoopCB(float elapsedMe, float elapsedSim, int counter, void * refcon);;

int MenuItem1;
// Widgets
XPWidgetID	EngineStarterWidget = NULL, EngineStarterWindow = NULL;
XPWidgetID	Description[MAX_NUMBER_ENGINES] = {NULL};
XPWidgetID	IgnitersState[MAX_NUMBER_ENGINES] = {NULL};
XPWidgetID	StartButton[MAX_NUMBER_ENGINES] = {NULL};
XPWidgetID	XPlaneStarterDurationEdit[MAX_NUMBER_ENGINES] = {NULL};
...
int Igniters[MAX_NUMBER_ENGINES] = {0};
int ElapsedTime[MAX_NUMBER_ENGINES] = {0};
int EngineStarting[MAX_NUMBER_ENGINES] = {0};

// Intitial window size
int x = 300, y = 500, w = 390, h = 290;

// Callback prototypes
void EngineStarterMenuHandler(void *, void *);

void CreateEngineStarterWidget(int x1, int y1, int w, int h);
						...
						long					inParam2)
{
	...
	if (inMessage == xpMsg_PushButtonPressed)
	{
		...
		for (Item=0; Item<MAX_NUMBER_ENGINES; Item++)
		{
			...
			if (inParam1 == (long)StartButton[Item])
			{
	...
	if (inMessage == xpMsg_ButtonStateChanged)
	{
		...
		for (Item=0; Item<MAX_NUMBER_ENGINES; Item++)
		{
			State = XPGetWidgetProperty(IgnitersState[Item], xpProperty_ButtonState, 0);
			Igniters[Item] = State;
		}
