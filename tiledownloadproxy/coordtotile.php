...
class Tile {
	...
	function getLatLongFromXYZoom($x,$y,$z) {
		$eps = 0.000001;
	    $zoomlevels=19;
	    $mapsize = 1 << $zoomlevels + log(256,2) - $z ;
	    
	    $origin = $mapsize/2.0;
	    $longdpp=360.0/$mapsize;
	    $long=-180.0+$x*256*$longdpp;
	
	    $latpprad=$mapsize/($this->PI*2.0);
	    $e=($y*256-$origin)/(-1.0*$latpprad);
	    if ($e > $this->PI)
	        $e = pi - $eps;
	    elseif ($e < (-1.0*$this->PI))
	        $e = (-1.0*$this->PI)+$eps;
	        
	    $lat=(2.0*atan(exp($e))-($this->PI/2.0))*180.0/$this->PI;
	    return new p($lat,$long);
	}
