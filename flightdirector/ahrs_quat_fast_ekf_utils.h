...
#define AHRS_QUAT_FAST_EKF_UTILS_H
...
#define AFE_QUAT_OF_EULER() {			                             \
    const double phi2     = afe_phi / 2.0;	                             \
    const double theta2   = afe_theta / 2.0;	                             \
    const double psi2     = afe_psi / 2.0;	                             \
						                             \
    ...
    const double costheta2 = cos( theta2 );	                             \
						                             \
    const double sinpsi2   = sin( psi2 );	                             \
    const double cospsi2   = cos( psi2 );	                             \
    									     \
...
#define AFE_NORM_QUAT() {			\
    double  mag = afe_q0*afe_q0 + afe_q1*afe_q1	\
...
#define AFE_COMPUTE_H_PHI() {          						\
    const double phi_err =  2 / (afe_dcm22*afe_dcm22 + afe_dcm12*afe_dcm12);     \
...
#define AFE_COMPUTE_H_THETA() {						        \
    const double theta_err  = -2 / sqrt( 1 - afe_dcm02*afe_dcm02 );	        \
...
#define AFE_COMPUTE_H_PSI() {					                \
  const double psi_err =  2 / (afe_dcm00*afe_dcm00 + afe_dcm01*afe_dcm01);       \
...
static inline double afe_phi_of_accel( const double* accel ) {
  return atan2(accel[AXIS_Y], accel[AXIS_Z]);
}

static inline double afe_theta_of_accel( const double* accel) {
  double g2 =					
 ...
 *
 */
static inline double afe_psi_of_mag( const int16_t* mag ) {
  const double ctheta  = cos( afe_theta );
#if 0
  const double    mn = ctheta * mag[0] 
    - (afe_dcm12 * mag[1] + afe_dcm22 * mag[2]) * afe_dcm02 / ctheta;
  
  const double    me =
    (afe_dcm22 * mag[1] - afe_dcm12 * mag[2]) / ctheta;
#else
  const double stheta  = sin( afe_theta );
  const double cphi  = cos( afe_phi );
  const double sphi  = sin( afe_phi );
