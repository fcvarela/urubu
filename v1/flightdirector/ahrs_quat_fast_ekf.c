 ...
 */
double afe_q0, afe_q1, afe_q2, afe_q3;
double afe_bias_p, afe_bias_q, afe_bias_r;

/* We maintain unbiased rates. */
double afe_p, afe_q, afe_r;

/* We maintain eulers */
double afe_phi, afe_theta, afe_psi;

/* time derivative of state
 * we know that bias_p_dot = bias_q_dot = bias_r_dot = 0
 */
double afe_q0_dot, afe_q1_dot, afe_q2_dot, afe_q3_dot;
 ...
 *
 */
void afe_predict( const double* gyro ) {
 ...
 * 
 */
static void run_kalman( const double R_axis, const double error ) {
  int i, j;
...
void afe_update_phi( const double* accel ) {
  AFE_COMPUTE_H_PHI();
  double accel_phi = afe_phi_of_accel(accel);
  double err_phi = accel_phi - afe_phi;
...
void afe_update_theta( const double* accel ) {
  AFE_COMPUTE_H_THETA();
  double accel_theta = afe_theta_of_accel(accel);
  double err_theta = accel_theta - afe_theta;
...
/* hacked for digital compass */
void afe_update_psi( const double mag ) {
  AFE_COMPUTE_H_PSI();
  double mag_psi = mag;//afe_psi_of_mag(mag);
  double err_psi = mag_psi - afe_psi;
 ...
 */
void afe_init( const double mag, const double* accel, const double* gyro ) {
  ...
  AFE_QUAT_OF_EULER();
  AFE_DCM_OF_QUAT();
  afe_psi = mag;//afe_psi_of_mag( mag );
